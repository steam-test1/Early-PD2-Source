...
function GroupAIStateBesiege:_spawn_in_group(spawn_group, spawn_group_type, grp_objective, ai_task)
	...
	while i <= #valid_unit_types do
		...
		if cat_data.special_type and cat_data.max_amount then
			if _get_special_unit_type_count(cat_data.special_type) + (spawn_entry.amount_min or 0) > cat_data.max_amount then
